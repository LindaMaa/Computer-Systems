...
int contain_diagonal(char *string, char *word) {
...
void strfind()
{
  ...
  for (int row = 0; row < maxrow; row++)
  {
    ...
    for (int col = 0; col < maxcol; col++)
    {
      ...
      {
        int offset = (row*get_grid_col_count())+col;
        word = dictionary + dictionary_idx[idx];

        if (contain(grid+offset, word))
        {
          printing(row,col, 'H', word);
          flag = 1;
        }
         if (contain_vertical(grid+offset, word))
        {
         printing(row,col, 'V', word);
          flag = 1;
          
        }
        if (contain_diagonal(grid+offset, word))
        {
